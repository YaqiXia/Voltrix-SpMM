...
from ..project import DEBUG_FLAG, PROJECT_NAME_FULL


# Name map for Python `eval`
typename_map: Dict[Any, str] = {
    **{t: t.__name__ for t in (bool, int)},
    torch.int: "torch.int",
...
ctype_map: Dict[Any, Any] = {
    **{t: getattr(ctypes, f"c_{t.__name__}") for t in (bool, int)},
    **{
        t: ctypes.c_void_p
        for t in (
            torch.int,
...
def generate(includes: Iterable[str], arg_defs: Iterable[Tuple], body: str) -> str:
    # Common prefix
    code = f"// {PROJECT_NAME_FULL} auto-generated JIT CUDA source file\n\n"
